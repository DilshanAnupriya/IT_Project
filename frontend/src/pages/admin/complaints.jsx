...
const AdminViewComplaintPage = () => {
  const [complaints, setComplaints] = useState([]);
  const [selectedComplaint, setSelectedComplaint] = useState(null);
  const [response, setResponse] = useState('');
  const [visibleCount, setVisibleCount] = useState(5); // State for number of visible complaints
  ...
  const fetchComplaints = async () => {
    try {
      ...
      setComplaints(res.data.complaints); // Correctly accessing the complaints from the response
    } catch (error) {
...
export default AdminViewComplaintPage;
